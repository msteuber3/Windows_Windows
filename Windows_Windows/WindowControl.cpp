...
WindowControl::WindowControl(HWND parent, HWND InstanceHandle, LPCWSTR oss, int x) : m_Minimize(NULL), m_Maximize(NULL), m_Close(NULL){
    m_Parent = parent;
    m_oss = oss;
...
void WindowControl::Create()
{
    ...
    HWND hControlPanel = CreateWindowEx(
        0,
        wc.lpszClassName, // Use a static control or any other class
        L"Control Panel",
        WS_CHILD | WS_VISIBLE | WS_BORDER,
        0, 0, 900, 900, // Position and size
    ...
    m_Title = CreateWindowEx(
        ...
        WS_CHILD | WS_VISIBLE,
        10, m_x + 10, 200, 30,
        hControlPanel, NULL, GetModuleHandle(NULL), NULL);
    ...
    m_Close = CreateWindowExW(
        ...
        wc.hInstance,
        NULL);
    SetWindowLongPtrW(hControlPanel, GWLP_USERDATA, (LONG_PTR)(m_InstanceHandle));
}
